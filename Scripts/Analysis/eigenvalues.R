...
name_lamda <- paste0("lambda_all_", period_char)
name_lamdaDominant <- paste0("lambda_dominant_", period_char)
...
for(chunk in 1:n_chunks){
  ...
  tau_harmonic_subs <- log(mean(tau_i_subs))
...
lamda <- mergeAndWrite(name_lamdaDominant, save = TRUE, datatype = "FLT4S")
cat("merging chunks tau Harmonic \n")
lamda <- mergeAndWrite(name_tauHarmonic, save = TRUE, datatype = "FLT4S")
