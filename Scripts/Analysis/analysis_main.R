...
sink(file.path(path_analysis_data_rast,"LM_int_overview.txt"))
subsample <- FALSE
plot <- FALSE
subSize <- 1000
lm_list <- list()
for(response in names(response_list)){
  ...
  for(interact in interaction_list){
    v1 <- interact[1]
    v2 <- interact[2]
    r <- c(response_list[[response]],predictor_list[[v1]], predictor_list[[v2]])
    cat("=====================================================================\n")
    ...
    lm_list_response[[v1]] <- lm_summary$coefficients
  }
  lm_list[[response]] <- lm_list_response
  cat("\n\n\n\n")
}

saveRDS(lm_list, file = file.path(path_analysis_data_rast, "lm_int_sum.rds"))
